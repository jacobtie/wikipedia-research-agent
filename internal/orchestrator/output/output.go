...
func (w *WriterTool) GetName() string {
	return "output_writer_tool"
}

func (w *WriterTool) GetDescription() string {
	return "The output_writer_tool writes the final output to a file once all research is complete"
}
...
func (w *WriterTool) Run(ctx context.Context, kwargs map[string]any) (string, error) {
	...
	filePath := path.Join("output", fmt.Sprintf("output_%d.txt", unixTimestamp))
